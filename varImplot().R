...
sample.split(city_temperature$Region,SplitRatio = 0.65)->split_tag1
subset(city_temperature,split_tag1==T)->train
subset(city_temperature,split_tag1==F)->test
nrow(train)
nrow(test)

randomForest(Region~AvgTemperature+Day+Month+Year, data = train, mtry=3, ntree=100)->mod_forest1
...
randomForest(Churn~tenure+StreamingTV+MonthlyCharges+TotalCharges, data = train, mtry=3, ntree=100)->mod_forest2
...
sample.split(Pharmacovigilance_audit_Data$DrugID,SplitRatio = 0.65)->split_tag3
subset(Pharmacovigilance_audit_Data,split_tag3==T)->train
subset(Pharmacovigilance_audit_Data,split_tag3==F)->test
nrow(train)
nrow(test)

randomForest(DrugID~Gender+Age+PatientID , data = train, mtry=3, ntree=100)->mod_forest3
